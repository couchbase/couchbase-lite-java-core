...
public final class Pusher extends Replication implements Database.ChangeListener {
    ...
    private void removePending(RevisionInternal revisionInternal) {
        ...
        if (!pendingSequences.contains(seq)) {
            Log.w(Log.TAG_SYNC, this + " removePending: sequence " + seq + " not in set, for rev " + revisionInternal);
        }
    ...
    void maybeCreateRemoteDB() {
        ...
        Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": maybeCreateRemoteDB() calling asyncTaskStarted()");
            ...
            public void onCompletion(Object result, Throwable e) {
                try {
                    ...
                    if(e != null && e instanceof HttpResponseException && ((HttpResponseException)e).getStatusCode() != 412) {
                        ...
                        Log.v(Log.TAG_SYNC, this + ": Created remote db");
                        createTarget = false;
                        beginReplicating();
                    }
                } finally {
                    Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": maybeCreateRemoteDB.onComplete() calling asyncTaskFinished()");
                    asyncTaskFinished(1);
                }
            }
    ...
    public void beginReplicating() {
        ...
        Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": beginReplicating() called");

        // If we're still waiting to create the remote db, do nothing now. (This method will be
        // re-invoked after that request finishes; see maybeCreateRemoteDB() above.)
        if(creatingTarget) {
            Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": creatingTarget == true, doing nothing");
            return;
        } else {
            Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": creatingTarget != true, continuing");
        }
        ...
        try {
            ...
            Log.w(Log.TAG_SYNC, "Error converting lastSequence: " + lastSequence + " to long.  Using 0");
        ...
        if(filterName != null && filter == null) {
            Log.w(Log.TAG_SYNC, String.format("%s: No ReplicationFilter registered for filter '%s'; ignoring", this, filterName));;
        }
    ...
    protected void processInbox(final RevisionList changes) {
        ...
        Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": processInbox() calling asyncTaskStarted()");
        Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": posting to /_revs_diff: " + diffs);
        ...
        sendAsyncRequest("POST", "/_revs_diff", diffs, new RemoteRequestCompletionBlock() {
            ...
            public void onCompletion(Object response, Throwable e) {
                ...
                try {
                    ...
                    Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": /_revs_diff response: " + response);
                    ...
                    Log.d(Log.TAG_SYNC, Pusher.this + "|" + Thread.currentThread() + ": processInbox() calling asyncTaskFinished()");
                    asyncTaskFinished(1);
                }
            }
    ...
    protected void uploadBulkDocs(List<Object> docsToSend, final RevisionList changes) {
        ...
        Log.v(Log.TAG_SYNC, String.format("%s: POSTing " + numDocsToSend + " revisions to _bulk_docs: %s", Pusher.this, docsToSend));
        ...
        Log.d(Log.TAG_SYNC, Pusher.this + "|" + Thread.currentThread() + ": uploadBulkDocs() calling asyncTaskStarted()");
        asyncTaskStarted();
            ...
            public void onCompletion(Object result, Throwable e) {
                try {
                    if (e == null) {
                        ...
                        for (Map<String, Object> item : items) {
                            ...
                            if (status.isError()) {
                                ...
                                Log.w(Log.TAG_SYNC, this + " _bulk_docs got an error: " + item);
                    ...
                    if (e != null) {
                        ...
                        Log.v(Log.TAG_SYNC, String.format("%s: POSTed to _bulk_docs: %s", Pusher.this, changes));
                    }
                    addToCompletedChangesCount(numDocsToSend);

                } finally {
                    Log.d(Log.TAG_SYNC, Pusher.this + "|" + Thread.currentThread() + ": uploadBulkDocs() calling asyncTaskFinished()");
                    asyncTaskFinished(1);
                }
    ...
    private boolean uploadMultipartRevision(final RevisionInternal revision) {
        ...
        for (String attachmentKey : attachments.keySet()) {
            ...
            if (attachment.containsKey("follows")) {
                ...
                if (inputStream == null) {
                    Log.w(Log.TAG_SYNC, "Unable to find blob file for blobKey: " + blobKey + " - Skipping upload of multipart revision.");
                    multiPart = null;
                }
                else {
        ...
        Log.d(Log.TAG_SYNC, "Uploading multipart request.  Revision: " + revision);
        Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": uploadMultipartRevision() calling asyncTaskStarted()");
            ...
            public void onCompletion(Object result, Throwable e) {
                try {
                    if(e != null) {
                        ...
                        Log.d(Log.TAG_SYNC, "Uploaded multipart request.  Result: " + result);
                        removePending(revision);
                    }
                } finally {
                    Log.d(Log.TAG_SYNC, this + "|" + Thread.currentThread() + ": uploadMultipartRevision() calling asyncTaskFinished()");
