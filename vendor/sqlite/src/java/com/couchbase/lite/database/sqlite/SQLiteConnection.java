...
import com.couchbase.lite.database.sqlite.exception.SQLiteBindOrColumnIndexOutOfRangeException;
import com.couchbase.lite.database.sqlite.exception.SQLiteDatabaseLockedException;
...
public final class SQLiteConnection implements CancellationSignal.OnCancelListener {
    private static final String TAG = Log.TAG_SQLiteConnection;
    ...
    private void warnOrthrowIfStatementForbidden(PreparedStatement statement) {
        if (mOnlyAllowReadOnlyOperations && !statement.mReadOnly) {
            if (mConfiguration.isOpenReadOnly()) {
                throw new SQLiteException("Cannot execute this statement (" + statement.mSql + ") "
                        + "because it might modify the database but the connection is read-only.");
            } else {
                Log.w(TAG, "The connection is acquired for a read-only statement. "
